<div class="container">
    <!-- <div class="row my-5">
        <div class="col text-center">
            <h1>Home</h1>
            <h1 class="line-1 anim-typewriter">Animation typewriter style using css steps()</h1>
        </div>
    </div> -->
    <!-- <div class="d-flex flex-column min-vh-100 justify-content-center align-items-left">
        <h1 class="line-1 anim-typewriter">Hey there, my name is Connor</h1>
    </div> -->
    <div class="d-flex flex-column min-vh-100 justify-content-center align-items-left">
        <div class="w-auto">
            <h1 class="mb-4">Hey, I'm Connor</h1>
            <p class="mb-4 text-muted">A software engineer specializing in full-stack web development</p>
            <a href="<?=App::route('/about')?>">
                <button class="btn btn-primary btn-rounded w-auto" style="width:auto !important;">More About Me...</button>
            </a>
        </div>
    </div>
</div>